...
#                 can_id = self.ReadBytes(RXBOSIDH, 2) # Read CAN ID
#                 pgn = ((can_id[0] << 8) | (can_id[1])) & 0x1FFC0 #Extract PGN from CAN id
#                 if pgn in pgns: #Check if extracted pgn is in list of pgns
#                     len = self.ReadByte(RXB0DLC) #Reads message length
#                     print(len)
#                     for i in range(len):
#                         CAN_RX_Buf.append(hex(self.ReadByte(RXB0D0 + i)))
#                     break
...
class ULISP():
    uart = machine.UART(1, baudrate=9600) #UART1
    
    def send(command, data):
        uart.write(bytes(f"{command} {data}\n", 'utf-8'))
...
if __name__ == '__main__':
    ...
    # INITIALISATION
    pgns = {
    "System Time": ['0x1F', '0x00', '0x00'],  # PGN 126992 (0x1F000): System Time
    "Vessel Heading": ['0x1F', '0x11', '0x20'],  # PGN 127250 (0x1F112): Vessel Heading
    "Rate of Turn": ['0x1F', '0x11', '0x21'],  # PGN 127251 (0x1F113): Rate of Turn
    "Attitude": ['0x1F', '0x11', '0x27'],  # PGN 127257 (0x1F119): Attitude
    "Engine Param(RU)": ['0x1F', '0x20', '0x00'],  # PGN 127488 (0x1F200): Engine Parameters, Rapid Update
    "Position(RU)": ['0x1F', '0x80', '0x01'],  # PGN 129025 (0x1F801): Position, Rapid Update
    "COG & SOG(RU)": ['0x1F', '0x80', '0x02'],  # PGN 129026 (0x1F802): COG & SOG, Rapid Update
    "GNSS Pos": ['0x1F', '0x80', '0x05'],  # PGN 129029 (0x1F805): GNSS Position Data
    "Time & Date": ['0x1F', '0x80', '0x09'],  # PGN 129033 (0x1F809): Time & Date
    "Wind Data": ['0x1F', '0x00', '0x02'],  # PGN 130306 (0x1F002): Wind Data
    }
    can = MCP2515()
    lisp = ULISP()
    print("init...")
    ...
    id = 0x1E0000 #max 7ff
    print("send data...")
    data = [0x24, 0x47, 0x50, 0x52, 0x4D, 0x43, 0x2C, 0x32, 0x32, 0x35]#, 0x34 0x34, 0x36, 0x2C, 0x41, 0x2C, 0x34, 0x39, 0x31, 0x36, 0x2E, 0x34, 0x35, 0x2C, 0x4E, 0x2C, 0x31, 0x32, 0x33, 0x31, 0x31, 0x2E, 0x31, 0x32, 0x2C, 0x57, 0x2C, 0x30, 0x30, 0x30, 0x2E, 0x35, 0x2C, 0x30, 0x35, 0x34, 0x2E, 0x37, 0x2C, 0x31, 0x39, 0x31, 0x31, 0x39, 0x34, 0x2C, 0x30, 0x32, 0x30, 0x2E, 0x33, 0x2C, 0x45, 0x2A, 0x36, 0x38]
    print(len(data))
    dlc = len(data)
...
#   From CAN
    ...
    while(1):
        ...
        #readbuf = ['0x24', '0x31', '0x30', '0x30', '0x30', '0x31', '0x2C', '0x31', '0x32', '0x36', '0x39', '0x39', '0x32', '2C', '0x31', '0x32', '2C', '0x32', '0x30', '32', '0x34', '2C', '0x30', 'x32', '2C', '0x30', 'x32', '2C', '0x31', '0x34', '2C', '0x33', '0x30', '2C', '0x30', '0x30', '2C', '0x31', '2C', '0x30', '2C', '0x30', 'x30', '2A', '0x36', '0x31']
        print(readbuf)
        found = False
        inp_pgn = readbuf[0:3]
        while not found:
            for pgn_name, pgn_val in pgns.items():
                if inp_pgn == pgn_val:
                    print("valid PGN found")
                    found = True
                    print("passed NMEA message to uLisp")
                    ToUlisp(pgn_name, readbuf)
                    break
        time.sleep(0.5)
        
#   To uLisp
    def ToUlisp(name, hex_dat):
        int_dat = [int(x,16) for x in hex_dat]
        binary_dat = bytes(int_dat)
        response = lisp.send(name, binary_dat)
